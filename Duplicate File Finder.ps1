# Ultimate Duplicate File Finder CLI (PowerShell Edition)
...
function Find-ExactDuplicates {
    ...
    foreach ($file in $candidates) {
        ...
        try {
            ...
            # Fix standard Object[] concatenation array type bug

            $combined = [byte[]]($startBytes + $endBytes)
...
try {
    ...
    # 1. UI PROMPT (Prompt only if Mode is prompt)
    ...
    if (-not $ffmpeg -and $Mode -eq "video") {
    ...
    if (-not $ffprobe -and $Mode -eq "video") {
    ...
    # 3. LOAD CACHE (For Video similarity)

    $cache = @{}

    if ($Mode -eq "video" -and (Test-Path $cacheFile)) {
    ...
    # 5. VIDEO MATCH ENGINE ROUTE

    Write-Host "`n🔍 Scanning videos..." -ForegroundColor Cyan
